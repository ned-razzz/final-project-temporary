...
def parse_args():
    parser = argparse.ArgumentParser(description="Render panel.html to a PDF file.")
    parser.add_argument("--input", default="panel.html", help="HTML file to render")
    parser.add_argument("--output", default="panel.pdf", help="PDF output path")
    parser.add_argument("--width", type=int, default=1500, help="Viewport width in CSS pixels")
    parser.add_argument("--height", type=int, default=2120, help="Minimum viewport height in CSS pixels")
    parser.add_argument("--dpi", type=int, default=300, help="Raster fallback PDF DPI")
    parser.add_argument(
        "--raster-scale",
        type=float,
        default=3,
        help="wkhtmltoimage fallback scale factor. Higher values preserve more detail.",
    )
    parser.add_argument(
        "--print-scale",
        type=float,
        default=1,
        help="Chrome PDF print scale. Must be between 0.1 and 2.",
    )
...
def wait_for_page_assets(websocket):
    websocket.call(
        "Runtime.evaluate",
        {
            "expression": (
...
def get_document_metrics(websocket):
...
def render_with_chrome(chrome, input_path, output_path, width, height, print_scale):
    ...
    try:
        ...
        websocket.call("Emulation.setEmulatedMedia", {"media": "screen"})
        websocket.call(
        ...
        wait_for_page_assets(websocket)
        metrics = get_document_metrics(websocket)
        page_width = max(width, int(metrics["width"]))
        page_height = max(height, int(metrics["height"]))
        pdf = websocket.call(
            "Page.printToPDF",
            {
                ...
                "marginBottom": 0,
                "marginLeft": 0,
                "paperWidth": page_width / CSS_PX_PER_INCH,
                "paperHeight": page_height / CSS_PX_PER_INCH,
                "scale": print_scale,
            },
            timeout=30,
        )
...
def render_with_wkhtmltopdf(wkhtmltopdf, input_path, output_path, width, height):
    cmd = [
        wkhtmltopdf,
        ...
        "0",
        "--page-width",
        f"{width / CSS_PX_PER_INCH}in",
        "--page-height",
        f"{height / CSS_PX_PER_INCH}in",
        "--viewport-size",
        f"{width}x{height}",
        input_path.as_uri(),
        str(output_path),
...
def render_with_wkhtmltoimage(wkhtmltoimage, input_path, output_path, width, raster_scale, dpi):
...
def main():
    ...
    if args.raster_scale <= 0:
        raise SystemExit("--raster-scale must be greater than 0.")
    if args.print_scale < 0.1 or args.print_scale > 2:
        raise SystemExit("--print-scale must be between 0.1 and 2.")

    chrome = shutil.which("google-chrome") or shutil.which("chromium") or shutil.which("chromium-browser")
    wkhtmltoimage = shutil.which("wkhtmltoimage")
    wkhtmltopdf = shutil.which("wkhtmltopdf")
    if not chrome and not wkhtmltoimage and not wkhtmltopdf:
        raise SystemExit("Chrome, Chromium, wkhtmltoimage, or wkhtmltopdf is required.")
    ...
    try:
        ...
        render_with_chrome(chrome, input_path, output_path, args.width, args.height, args.print_scale)
    except (subprocess.CalledProcessError, RuntimeError, TimeoutError):
        if wkhtmltoimage:
            render_with_wkhtmltoimage(
                wkhtmltoimage,
                input_path,
                output_path,
                args.width,
            ...
            render_with_wkhtmltopdf(wkhtmltopdf, input_path, output_path, args.width, args.height)
        else:
            raise
